...
export function useMenuItems() {
  ...
  function handleManageUsers() {
    router.push(PATHNAME.MANAGE_USERS)
  }
  ...
  const manageUsers: MenuItem = { handler: handleManageUsers, label: '계정 관리', icon: FaUsersCog }

  const menuItems: { [key in USER_TYPE]: MenuItem[] } = {
    [USER_TYPE.SUPER]: [setCalendar, register, manageUsers, changePassword, logout],
